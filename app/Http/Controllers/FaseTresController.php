...
class FaseTresController extends Controller
{
    ...
    public function storeFileInformeDos(Request $request)
    {
        ...
        try {
            ...
            if ($request->hasFile('archivo')) {
                ...
                if ($archivo->isValid() && $archivo->getClientOriginalExtension() === 'pdf') {
                    ...
                    $nombreArchivo = 'informeDos'.$matricula . '.pdf';
                    ...
                    try {
                        ...
                        $existeRegistro = FaseTres::where('id_servicio', $id_servicio)->where('reporte_dos', '!=', null)->first();
            
                        if (!$existeRegistro) {
                            // Crear una nueva instancia de FaseUno y guardarla en la base de datos
                            $fasetres = new FaseTres();
                            $fasetres->reporte_dos = $nombreArchivo;
                            $fasetres->id_servicio = $id_servicio;
                            $fasetres->comentario="";
                            $fasetres->estatus_envio=1;
                            $fasetres->save();
                        }
    ...
    public function cambiarEstadoInforme2($estado)
    {
        try {
            ...
            return ApiResponses::success('Estado cambiado correctamente', 200, $faseTres->estatus_envio);
    ...
    public function getInformeDos($nombreArchivo)
{
    $directorio = 'public';
...
public function cambiarEstatusInforme2($matricula, $estado)
    {
        try {
    ...
    public function enviarComentarioInforme2($matricula, $comentario)
    {
        try {
    ...
    $faseTres = FaseTres::where('id_servicio', $id_ser)->firstOrFail();
            ...
            $faseTres->comentario = $comentario;
            $faseTres->save();
    
            return ApiResponses::success('Comentario Enviado', 200, $faseTres->comentario);
        } catch (ModelNotFoundException $ex) {
            return ApiResponses::error('Estudiante no encontrado', 404);
        } catch (Exception $e) {
            return ApiResponses::error('Error: ' . $e->getMessage(), 500);
